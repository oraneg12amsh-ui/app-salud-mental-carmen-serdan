...
export default function App() {
  ...
  useEffect(() => {
    signInAnonymously(auth).catch((error) => {
      ...
      setDbError("Error de conexión. Verifica tu internet.");
    });
  }, []);
  ...
  const finishSession = async () => {
    ...
    const newSubmission = {
      ...
      mesactsScore,
      tmmsScore,
      createdAt: timestamp.toISOString() // For sorting in Firebase
    };
    ...
    try {
      ...
      alert("Hubo un error al guardar los resultados. Por favor avisa a tu profesor.");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 relative">
      ...
      <div className="text-center max-w-2xl w-full animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <p className="text-base md:text-lg text-slate-600 mb-10 max-w-lg mx-auto leading-relaxed px-4">
          ...
          <br/>
          <span className="text-sm text-slate-400 block mt-2">Escuela Secundaria General "Carmen Serdán"</span>
        </p>
        ...
        <div className="bg-white max-w-3xl w-full rounded-none shadow-2xl p-6 md:p-12 border border-slate-200 relative">
          ...
          <div className="font-serif text-slate-800 space-y-6 leading-relaxed text-justify text-sm md:text-base h-80 md:h-96 overflow-y-auto pr-2 custom-scrollbar">
            ...
            <p>
              ...
              quien cursa el nivel de educación secundaria en la <span className="font-bold">Escuela Secundaria General "Carmen Serdán"</span>, 
      ...
      >
        ...
        <div className="mt-12 flex justify-end pb-12">
          <Button 
            ...
            className={!isComplete || isSaving ? "opacity-50 cursor-not-allowed" : "shadow-xl transform hover:-translate-y-1 transition-transform"}
          >
            {isSaving ? "Guardando..." : "Finalizar Todo"} 
            {!isSaving && <CheckCircle2 size={16} className="ml-2 inline" />}
          </Button>
        </div>
      </TestLayout>
    );
